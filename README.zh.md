...
如果选错评分，桌面端可按 **U**，移动端可摇一摇手机，恢复当前复习 session 中最近一次评分，并回到那张卡片。移动端可能需要先授权动作传感器权限，摇一摇撤回才会生效。撤回只作用于当前打开的复习 session；如果笔记已被外部修改，或另一台设备的 iCloud 同步尚未完成，插件会拒绝撤回，避免把笔记回滚到错误状态。
