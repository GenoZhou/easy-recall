...
在插件设置中将「点击逐项复习」设为「仅桌面端」「仅移动端」「启用」或「关闭」，即可在对应平台逐个自测挖空答案。每个隐藏挖空项点按后会按隐藏、显示、删除线循环。当还有挖空项处于隐藏状态时，不会显示「显示答案」按钮。所有挖空项都进入显示或删除线状态后，会自动出现评分确认按钮（「没记住」「有点难」或「记住了」），评分按显示项比例计算。默认阈值为「有点难」50%、「记住了」80%，可在设置中自定义。该模式默认关闭；启用时除有提示可用空格显示提示外，不启用显示答案或评分快捷键。
