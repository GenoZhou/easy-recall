...
使用命令面板执行「切换点击逐项复习」（或在 Obsidian 设置 → 快捷键中为其绑定快捷键）可快速全局开启或关闭点击逐项复习模式。该设置仅对新打开的复习会话生效。
...
在插件设置中开启「点击逐项复习」，可以逐个自测挖空答案。每个隐藏挖空项点按后会按隐藏、显示、删除线循环。当还有挖空项处于隐藏状态时，不会显示「显示答案」按钮。所有挖空项都进入显示或删除线状态后，会自动出现评分确认按钮（「没记住」「有点难」或「记住了」），评分按显示项比例计算。默认阈值为「有点难」50%、「记住了」80%，可在设置中自定义。该模式默认关闭，且除有提示时可用空格显示提示外，不启用显示答案或评分快捷键。
